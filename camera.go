...
func defaultCamera(lookfrom vec3, lookat vec3, vup vec3, vfov float32, aspect float32, aperture float32, focusDist float32) *camera {
...
func randomInUnitDisk() vec3 {
	var p vec3
	for {
		p = vec3{rand.Float32(), rand.Float32(), 0}.multiplyBy(2.0).minus(vec3{1.0, 1.0, 0.0})
		if p.dot(p) < 1.0 {
			break
		}
	}
